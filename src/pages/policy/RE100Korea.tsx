...
export default function RE100Korea() {
  ...
  const columns = [
    ...
    { key: 'sector', label: '업종', render: (v: unknown) => <span className="text-xs text-[#8B949E]">{String(v)}</span> },
    ...
    { key: 'current_pct', label: '달성률', sortable: true, align: 'right' as const,
      render: (v: unknown, row: Record<string, unknown>) => (
        <div className="flex items-center gap-2 justify-end">
          <Progress value={v as number} className="w-16 h-1.5" barClassName={v as number >= 50 ? 'bg-[#2EA043]' : 'bg-[#388BFD]'} />
          <span className="text-xs font-bold text-[#E6EDF3]">{String(v)}%</span>
        </div>
      )
    },
  ]
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KPICard title="RE100 가입 기업" value={re100Companies.length} unit="개사" icon={Building2} iconColor="#2EA043" />
        <KPICard title="평균 달성률" value={avgProgress} unit="%" icon={TrendingUp} iconColor="#388BFD" />
        <KPICard title="50% 이상 달성" value={above50} unit="개사" icon={CheckCircle2} iconColor="#D29922" />
      ...
      <ChartWrapper
        ...
        actions={
          <div className="flex gap-2">
            <button onClick={() => setSortBy('pct')} className={`text-xs px-2 py-0.5 rounded ${sortBy === 'pct' ? 'bg-[#21262D] text-[#E6EDF3]' : 'text-[#8B949E]'}`}>달성률순</button>
            <button onClick={() => setSortBy('year')} className={`text-xs px-2 py-0.5 rounded ${sortBy === 'year' ? 'bg-[#21262D] text-[#E6EDF3]' : 'text-[#8B949E]'}`}>가입순</button>
          </div>
        }
      >
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={barData}>
            ...
            <XAxis dataKey="company" {...DARK_AXIS} tick={{ fill: '#8B949E', fontSize: 10 }} />
      ...
      <div className="bg-[#161B22] border border-[#30363D] rounded-lg overflow-hidden">
        <div className="px-4 py-3 border-b border-[#30363D] flex items-center justify-between">
          <h3 className="text-sm font-semibold text-[#E6EDF3]">RE100 가입 기업 상세</h3>
      ...
      <div className="bg-[#161B22] border border-[#30363D] rounded-lg p-4">
        <h3 className="text-sm font-semibold text-[#E6EDF3] mb-3">RE100 이행 수단 (한국)</h3>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {[
            { title: '재생에너지 구매계약(PPA)', desc: '발전사업자와 직접 계약. 비용 절감 + RE100 인정', color: '#2EA043', pct: 45 },
            { title: 'REC 구매', desc: 'REC 현물·계약시장 거래. 유연성 높음', color: '#388BFD', pct: 30 },
            { title: '한국형 PPA (제3자)', desc: '중간 플랫폼 통한 간접 PPA. 2023년 도입', color: '#D29922', pct: 15 },
            ...
            <div key={i} className="bg-[#0D1117] rounded-lg p-3">
              <div className="flex justify-between items-start mb-2">
                <span className="text-xs font-medium text-[#E6EDF3]">{item.title}</span>
              ...
              <p className="text-[10px] text-[#6E7681]">{item.desc}</p>
            </div>
          ))}
        </div>
      </div>
