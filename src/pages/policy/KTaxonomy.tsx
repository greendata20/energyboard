  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KPICard title="녹색부문 경제활동" value="69" unit="개 활동" icon={Leaf} iconColor="var(--color-primary)" />
        <KPICard title="전환부문 활동" value="6" unit="개 활동" icon={Zap} iconColor="var(--color-amber)" />
        <KPICard title="총 분류 활동" value="75" unit="개" icon={Building2} iconColor="var(--color-electric)" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-primary-muted border border-primary rounded-lg p-4">
          <div className="text-sm font-semibold text-primary-hover mb-1">6대 환경목표</div>
          <div className="grid grid-cols-2 gap-2 mt-2">
            {[
              ...
              <div key={i} className="bg-bg-base rounded px-2 py-1.5 text-xs text-text-primary">{goal}</div>
            ))}
          </div>
        </div>
        <div className="bg-amber-muted border border-amber rounded-lg p-4">
          <div className="text-sm font-semibold text-amber-hover mb-2">4대 인정 기준</div>
          <div className="space-y-2 text-xs text-text-primary">
            <div className="flex gap-2"><span className="text-amber">1.</span> 6대 환경목표 중 하나 이상에 <strong>기여</strong></div>
            <div className="flex gap-2"><span className="text-amber">2.</span> 나머지 5개 목표에 <strong>심각한 피해 없음</strong> (DNSH)</div>
            <div className="flex gap-2"><span className="text-amber">3.</span> <strong>최소 사회 안전장치</strong> 준수</div>
            <div className="flex gap-2"><span className="text-amber">4.</span> 기술 심사 기준 <strong>충족</strong></div>
          </div>
        </div>
      </div>
      ...
      <Tabs defaultValue="green">
        ...
        <TabsContent value="green">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {taxonomyData.green.categories.map((cat, i) => (
              <div key={i} className="bg-bg-card border border-primary/30 rounded-lg p-4">
        ...
        <TabsContent value="transition">
          <div className="mb-3 bg-amber-muted border border-amber rounded-lg p-3">
            <p className="text-xs text-amber-hover">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {taxonomyData.transition.categories.map((cat, i) => (
              <div key={i} className="bg-bg-card border border-amber/30 rounded-lg p-4">
                <div className="flex items-center gap-2 mb-3">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  <th className="px-3 py-2.5 text-left text-text-secondary">비교 항목</th>
                  <th className="px-3 py-2.5 text-left text-primary">K-Taxonomy (한국)</th>
                  <th className="px-3 py-2.5 text-left text-electric">EU Taxonomy</th>
                </tr>
              </thead>
              <tbody>
                {comparisonEU.map((row, i) => (
                  <tr key={i} className="border-b border-border-subtle">
                    <td className="px-3 py-2.5 text-text-secondary font-medium">{row.aspect}</td>
                    <td className="px-3 py-2.5 text-text-primary">{row.korea}</td>
                    <td className="px-3 py-2.5 text-text-primary">{row.eu}</td>
                  </tr>
                ))}
              </tbody>
            </table>
