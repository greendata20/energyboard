    ...
    <div className="space-y-6">
      ...
      <div className="bg-danger-muted border border-danger rounded-lg p-4">
        <div className="flex items-start gap-3">
          <AlertTriangle size={18} className="text-danger-hover shrink-0 mt-0.5" />
          <div>
            <div className="text-sm font-semibold text-danger-hover mb-1">CBAM 주요 일정</div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KPICard title="총 CBAM 노출액" value={totalExposure} unit="백만 EUR/년" icon={DollarSign} iconColor="var(--color-danger)" />
        <KPICard title="철강 부담 비중" value={((steelExposure / totalExposure) * 100).toFixed(0)} unit="%" icon={Globe} iconColor="var(--color-amber)" />
        <KPICard title="EU ETS 탄소가격" value={65} unit="EUR/tCO₂" icon={TrendingUp} iconColor="var(--color-electric)" />
          ...
          <div className="space-y-4 mt-4">
            <div>
              <div className="flex justify-between text-xs mb-2">
                <span className="text-text-secondary">K-ETS (한국)</span>
                <span className="text-text-primary font-bold">8,500원/tCO₂ ≈ €6/tCO₂</span>
              </div>
              <div className="w-full bg-bg-elevated rounded-full h-3">
                <div className="h-3 rounded-full bg-electric" style={{ width: `${(6 / 65) * 100}%` }} />
              </div>
            </div>
            <div>
              <div className="flex justify-between text-xs mb-2">
                <span className="text-text-secondary">EU ETS (유럽)</span>
                <span className="text-text-primary font-bold">€65/tCO₂</span>
              </div>
              <div className="w-full bg-bg-elevated rounded-full h-3">
                <div className="h-3 rounded-full bg-danger" style={{ width: '100%' }} />
              </div>
            </div>
            <div className="bg-bg-base rounded-lg p-3">
              <div className="text-xs text-text-secondary mb-1">탄소가격 격차 (CBAM 조정 필요)</div>
              <div className="text-2xl font-bold text-danger">€59/tCO₂</div>
              <div className="text-xs text-text-secondary mt-1">한국 기업이 EU 수출 시 추가 부담해야 할 탄소 비용</div>
            </div>
            <div className="space-y-1.5">
              {cbamData.map(d => (
                <div key={d.sector} className="flex items-center justify-between bg-bg-base rounded-md px-3 py-1.5">
                  <span className="text-xs text-text-primary">{d.sector}</span>
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-text-secondary">{d.carbon_intensity} tCO₂/t</span>
                    <span className="text-xs text-text-secondary">{formatNumber(d.export_kt)} kt</span>
