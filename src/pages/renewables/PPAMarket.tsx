  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KPICard title={`${latest.year} PPA 계약 건수`} value={latest.ppa_contracts} unit="건" icon={FileCheck} iconColor="#2EA043" />
        <KPICard title="총 계약용량" value={formatNumber(latest.total_mw)} unit="MW" icon={Zap} iconColor="#388BFD" />
        <KPICard title="PPA vs 한전 가격차" value={priceDiff} unit="원/kWh 저렴" icon={DollarSign} iconColor="#D29922" />
      ...
      <div className="bg-[#1A3F24] border border-[#2EA043] rounded-lg p-4">
        <div className="text-sm font-semibold text-[#3FB950] mb-1">PPA 경제성 포인트</div>
        <div className="text-xs text-[#E6EDF3]">
          {latest.year}년 기준 PPA 평균단가 {latest.avg_price_kwh}원/kWh vs 한전 산업용(을) {latest.kepco_industrial_kwh}원/kWh → <strong className="text-[#3FB950]">{priceDiff}원/kWh 절감</strong>,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-[#161B22] border border-[#30363D] rounded-lg p-4">
          <h3 className="text-sm font-semibold text-[#E6EDF3] mb-3">연도별 PPA 시장 성장</h3>
          <div className="space-y-4">
            {ppaData.map((d, i) => (
              <div key={i}>
                <div className="flex justify-between text-xs mb-1.5">
                  <span className="text-[#8B949E]">{d.year}년</span>
                  <span className="text-[#E6EDF3]">{d.ppa_contracts}건 · {formatNumber(d.total_mw)} MW</span>
                </div>
                <div className="w-full bg-[#21262D] rounded-full h-2">
                  <div
                    className="h-2 rounded-full bg-[#2EA043]"
          ...
          <div className="mt-4 text-xs text-[#8B949E]">
            <div className="font-semibold text-[#E6EDF3] mb-2">PPA 도입 주요 RE100 한국 기업</div>
