  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KPICard title="최근 REC 단가" value={formatNumber(latest.price_krw)} unit="원/REC" icon={DollarSign} iconColor="var(--color-primary)" />
        <KPICard title="연평균 단가" value={formatNumber(avgPrice)} unit="원/REC" icon={BarChart2} iconColor="var(--color-electric)" />
        <KPICard title="연간 총 거래량" value={formatNumber(totalVolume)} unit="REC" icon={Zap} iconColor="var(--color-amber)" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-bg-card border border-border rounded-lg p-4">
          <h3 className="text-sm font-semibold text-text-primary mb-3">REC 가중치 주요 기준</h3>
          <div className="space-y-2">
            {[
              { source: '육상풍력 (일반)', weight: '1.0', color: 'var(--color-primary)' },
              { source: '태양광 (100kW 미만)', weight: '1.2', color: 'var(--color-electric)' },
              { source: '수상태양광', weight: '1.5', color: 'var(--color-amber)' },
              { source: '해상풍력 (연계거리 5km↑)', weight: '3.5', color: '#A371F7' },
              { source: '연료전지', weight: '2.0', color: '#39C5CF' },
              { source: 'ESS 연계 태양광 (야간)', weight: '5.0', color: 'var(--color-danger)' },
            ].map((item, i) => (
              <div key={i} className="flex items-center justify-between py-1.5 border-b border-border-subtle">
        ...
        <div className="bg-bg-card border border-border rounded-lg p-4">
          <h3 className="text-sm font-semibold text-text-primary mb-3">RPS 의무 이행 현황 (2024)</h3>
            ...
            {[
              ...
              <div key={i}>
                <div className="flex justify-between text-xs mb-1">
                  <span className="text-text-secondary">{item.supplier}</span>
                  <span className={item.deficit ? 'text-danger-hover' : 'text-primary-hover'}>
                ...
                <div className="w-full bg-bg-elevated rounded-full h-1.5">
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor: item.deficit ? 'var(--color-danger)' : 'var(--color-primary)',
                    }}
                  />
                </div>
              </div>
          ...
          <p className="text-xs text-text-muted mt-3">* 2024년 RPS 의무비율: 13%</p>
        </div>
      </div>
    </div>
  )
